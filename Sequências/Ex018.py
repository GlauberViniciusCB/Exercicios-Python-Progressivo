...
def analise():
    ...
    elif(numero > 23  or numero <0 ):
        print("Número Invalido, Por Favor Digite Um Número Entre 1 E 23.")
        print("\n")
        
    else:
        armazemdevoto.append(numero) 
        print("Voto Computado Com Sucesso!")
        print("\n")
        

print("\n---------------------------------------------------------------------------------------------")
print("                                  Olá,Seja Bem Vindo A Enquete!\n          Por Favor Escolha Um Número Entre 1 A 23, Caso Queira Encerrar A Votação Digite 0.          ")
print("---------------------------------------------------------------------------------------------")

armazemdevoto=[]
numero = int(input("Informe Um Número Da  Camisa Do Jogador:"))
analise()
    
while(numero != 0):
    numero = int(input("Informe Outro Número De  Camisa Do Jogador:"))
    analise()


print("Foram Computados",len(armazemdevoto),"Votos")

for i in range (1,24):
    if(armazemdevoto.count(i)!=0):
        print("O Jogador",i,"Recebeu",armazemdevoto.count(i),"Voto(s), Que Correspondem A ", 100*(armazemdevoto.count(i)/len(armazemdevoto)),"% Dos Votos Computados")      


for j in (armazemdevoto):
    if(armazemdevoto.count(j) > vencedor):
        vencedor = armazemdevoto.count(j)
        mvp = j
print ("\nFim Da Votação Já Temos Um Resultado:")
print("O Vencedor É O Jogador de Camisa: ",mvp)